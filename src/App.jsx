...
async function consultarAlertasHoy() {
  const url = `${SUPABASE_URL}/rest/v1/alertas?fecha=eq.${today()}&select=tipo,horario,cgm,categoria&order=horario.desc`;
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#17110a", border: "1px solid #2a1f12", borderRadius: 16, maxWidth: "min(1000px, 96vw)", width: "100%", maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}
      >
...
function TablaHistorial({ columnas, filas }) {
  return (
    <table style={{ width: "100%", tableLayout: "fixed", borderCollapse: "collapse", fontFamily: "'IBM Plex Mono', monospace", fontSize: "0.78rem" }}>
      <thead>
        <tr style={{ borderBottom: "1px solid #2a1f12" }}>
          {columnas.map((c) => (
            <th key={c} style={{ textAlign: "left", padding: "8px 8px", color: "#7a6040", fontFamily: "'Syne', sans-serif", fontSize: "0.66rem", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", wordBreak: "break-word" }}>{c}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {filas.map((fila, i) => (
          <tr key={i} style={{ borderBottom: "1px solid rgba(42,31,18,0.6)" }}>
            {fila.map((val, j) => (
              <td key={j} style={{ padding: "8px 8px", color: "#e8d5b0", wordBreak: "break-word" }}>{val || "—"}</td>
            ))}
          </tr>
        ))}
      </tbody>
...
export default function AlertasApp({ onVolver }) {
  ...
  async function cargarHoy() {
    ...
    setHoyError(null);
    try {
      const data = await consultarAlertasHoy();
      setHoyRegistros(data);
    } catch (err) {
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: hoyLoading || hoyError || (hoyRegistros && hoyRegistros.length) ? 10 : 0 }}>
              <span style={{ fontFamily: "'Syne', sans-serif", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#7a6040" }}>
                Alertas cargadas hoy
              </span>
              <button onClick={cargarHoy} disabled={hoyLoading} style={{ background: "none", border: "none", color: "#f59e0b", cursor: hoyLoading ? "default" : "pointer", fontSize: 11, fontFamily: "'Syne', sans-serif", fontWeight: 700 }}>
                ↺ Actualizar
              </button>
            </div>
            {hoyLoading && <p style={{ fontSize: "0.8rem", color: "#7a6040", fontFamily: "'Syne', sans-serif" }}>Cargando…</p>}
            {hoyError && <p style={{ fontSize: "0.8rem", color: "#fca5a5", fontFamily: "'Syne', sans-serif" }}>{hoyError}</p>}
            {!hoyLoading && !hoyError && hoyRegistros && hoyRegistros.length === 0 && (
              <p style={{ fontSize: "0.8rem", color: "#7a6040", fontFamily: "'Syne', sans-serif" }}>Todavía no se cargó ninguna alerta hoy.</p>
            )}
